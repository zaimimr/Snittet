...
export default function Card({ course }: { course: ApiUserCourse_CourseType }) {
    ...
    async function deleteCard(course: string, user_id: string) {
        "use server"
        await sql.query(`DELETE FROM user_course WHERE user_id = '${user_id}' AND course = '${course}'`)
        revalidatePath("/")
    }

    async function changeGradeType(course: string, user_id: string) {
        "use server"
        await sql.query(`UPDATE user_course SET is_grade = NOT is_grade WHERE user_id = '${user_id}' AND course = '${course}'`)
        revalidatePath("/")
    }
                    ...
                    <div className="text-error-main hover:text-green-500 cursor-pointer" >
                        {
                            user_id && <CardActionButton child={
                                <svg className="stroke-white w-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M18 4L21 7M21 7L18 10M21 7H7C4.79086 7 3 8.79086 3 11M6 20L3 17M3 17L6 14M3 17H17C19.2091 17 21 15.2091 21 13" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                            } actionFunction={changeGradeType} course={course.course} user_id={user_id?.value} />
                        }
                    </div>
                    <div className=" text-error-main hover:text-green-500 cursor-pointer" >
                        {
                            ...
                            } actionFunction={deleteCard} course={course.course} user_id={user_id?.value} />
                        }
                    </div>
                </div>
